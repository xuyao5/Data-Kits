...
public final class File2EsExecutor extends AbstractExecutor {
    ...
    public <T extends BaseDocument> void execute(File2EsConfig config, EventFactory<T> document, UnaryOperator<T> operator) {
        //检查文件和索引是否存在
    ...
    @SneakyThrows
    private void publish(Disruptor<StandardFileLine> disruptor, File file, Charset charset) {
