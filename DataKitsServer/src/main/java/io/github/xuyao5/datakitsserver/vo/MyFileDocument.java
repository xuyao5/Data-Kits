...
public final class MyFileDocument extends BaseDocument {
    ...
    @AutoMappingField(position = 0)//column = "UUID",
    private String uuid;

    @SerializedName(value = "cashAmount", alternate = {"CashAmount"})
    @AutoMappingField(position = 1)//column = "CASH_AMOUNT",
    private BigDecimal cashAmount;

    @SerializedName(value = "desc1", alternate = {"Desc1"})
    @AutoMappingField(position = 2, sortPriority = -1, order = DESC)//column = "DESC1"
    private String desc1;

    @SerializedName(value = "desc2", alternate = {"Desc2"})
    @AutoMappingField(position = 3, sortPriority = -1, order = DESC)//column = "DESC2"
    private StringBuilder desc2;

    @SerializedName(value = "dateTime1", alternate = {"DateTime1"})
    @AutoMappingField(position = 4, sortPriority = 2, order = ASC)//column = "DATE_TIME_1"
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
    private long dateTime1;

    @SerializedName(value = "dateTime2", alternate = {"DateTime2"})
    @AutoMappingField(position = 5)//column = "DATE_TIME_2"
    @JsonFormat(pattern = "yyyy-MM-dd HH:mm:ss")
    private Date dateTime2;

    @SerializedName(value = "discount1", alternate = {"Discount1"})
    @AutoMappingField(sortPriority = 3, order = DESC)//
    ...
    @AutoMappingField(position = 6)//column = "GEO"
    private GeoPoint position;

    @SerializedName(value = "location", alternate = {"Location"})
    private GeoPoint location;//大多数场景可以不需要@AutoMappingField转换，通过内建的计算引擎直接new一个GeoPoint
