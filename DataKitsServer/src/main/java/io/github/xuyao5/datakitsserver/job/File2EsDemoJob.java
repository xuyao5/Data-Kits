...
//        file2EsTasks.seek(taskId).ifPresent(File2EsExecutor.builder().build()::execute);
        File2EsConfig config = File2EsConfig.of(new File("/Users/xuyao/Downloads/DISRUPTOR_100W_T_00.txt"), "file2es_disruptor_1", "file2es_disruptor");
        new File2EsExecutor(esClient).execute(MyDocument::of, config);
    }
}