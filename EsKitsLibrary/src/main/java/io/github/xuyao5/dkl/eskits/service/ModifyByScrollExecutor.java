...
public final class ModifyByScrollExecutor extends AbstractExecutor {
    ...
    public <T extends BaseDocument> void computeByScroll(@NonNull ModifyByScrollConfig config, EventFactory<T> document, Consumer<T> compute) {
        DisruptorBoost.<T>factory().create().processOneArg(consumer -> eventConsumer(config, consumer), (sequence, t) -> log.error(t.toString()), document, (standardDocument, sequence, endOfBatch) -> compute.accept(standardDocument));
    }
