 ...
 */
@Slf4j
public final class File2EsExecutor2 extends AbstractExecutor {

    private final int bulkThreads;

    public File2EsExecutor2(@NonNull RestHighLevelClient esClient, int threads) {
        super(esClient);
        bulkThreads = threads;
    }
    ...
    public <T extends BaseDocument> void execute(@NonNull File2EsConfig config, EventFactory<T> document, UnaryOperator<T> operator) {
        ...
        BulkSupporter.getInstance().bulk(client, bulkThreads, function -> DisruptorBoost.<StandardFileLine>factory().create().processTwoArg(consumer -> {
        ...
        }, StandardFileLine::of, (standardFileLine, sequence, endOfBatch) -> {
